...
export default function Component() {
  ...
  useEffect(() => {
    ...
    function createTextImage() {
      ...
      ctx.fillText(comingSoonText, canvas.width / 2, canvas.height / 2)
      ...
      for (let attempt = 0; attempt < 100; attempt++) {
        ...
        if (data[(y * canvas.width + x) * 4 + 3] > 128) {
          return {
            ...
            color: "white",
            scatteredColor: "#4ECDC4", // Teal color for all particles
    ...
    function animate(scale: number) {
      ...
      // Add null check for canvas before calculating targetParticleCount
      const targetParticleCount = canvas ? Math.floor(
      ...
      {/* <div className="absolute bottom-[100px] text-center z-10">
        <p className="font-mono text-gray-400 text-xs sm:text-base md:text-sm ">
          ...
          <a
            href="#"
            target="_blank"
            className="invite-link text-gray-300 hover:text-[#4ECDC4] transition-colors duration-300"
            rel="noreferrer"
          >
            waitlist
          </a>{" "}
          <span>for</span>
          <span className="transition-colors duration-300"> early access</span>
        </p>
      </div> */}
    </div>
  )
